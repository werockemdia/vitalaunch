...
class MultiSiteCloner {
    ...
    private function get_first_blog_id() {
        global $wpdb;
        $query = $wpdb->prepare ( "SELECT `blog_id` FROM `$wpdb->blogs` WHERE `blog_id` != %d ORDER BY `blog_id` ASC LIMIT 0,1", $this->get_main_blog_id() );
        return $wpdb->get_var ( $query );
    }
    ...
    function wp_mu_clone_settings() {
        ...
        <div class="wrap">
            ...
            <h1>Multisite Cloner Settings</h1>
            <form method="post">
                <?php settings_fields( 'default' ); ?>
                <h2>Default blog to be cloned</h2>
