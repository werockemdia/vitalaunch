...
function wooc_extra_register_fields() {?>
       ...
       <label for="reg_billing_full_name"><?php _e( 'Full name', 'woocommerce' ); ?><span class="required">*</span></label>

       <input type="text" class="woocommerce-Input woocommerce-Input--text input-text" name="billing_full_name" id="reg_billing_full_name" placeholder="Jhon Deo" value="<?php if ( ! empty( $_POST['billing_full_name'] ) ) esc_attr_e( $_POST['billing_full_name'] ); ?>" required/>
...
function pfwp_redirect_woo_checkout( $order_id ){
    ...
    if( ! get_post_meta( $order_id, '_thankyou_action_done', true ) ) {
      ...
      $user_id = get_current_user_id();
        ...
        $meta_values = $wpdb->get_results("SELECT * FROM  wp_wc_orders_meta  WHERE (meta_key = '_stripe_customer_id' AND order_id = '".$order_id."')");
 
$cus_id =  $meta_values[0]->meta_value;
 global $current_user;
wp_get_current_user();
...
// try code
        ...
        foreach ( $order->get_items() as $item_id => $item ) {
           ...
           $product_ids = $product->get_id();
             ...
             //if($m_value >= 50){
                $user_id = get_current_user_id();
                $post_ids = $wpdb->get_results("SELECT * FROM `marge_img` WHERE `user_id` = '".$user_id."' AND `product_id` = '".$product_ids."' ORDER BY ID DESC LIMIT 1 ");
...
function mystore_endpoint_content() {
    ...
    <a class="startbutton" target="_blank" href="https://<?php echo $store_own; ?>/wp-admin/tools.php?page=domainmapping"><button>Domain Setup</button></a><br><br>
     ...
     <div class="col-md-6">
        <video style="width:100%;height:300px;object-fit:cover;" loop="true" autoplay="autoplay" muted>
...
.f_l_name #menu ul.dashMenu li a {
    ...
    foreach($sc as $scs){
     ...
     <nav id="menu">
      ...
      $message = '<a class="logBtn" href="https://vitalaunch.io/login/">Log in</a> <a class="singupBtn" href="/memberships/">Sign up</a>';
  }
return $message;
}
// register shortcode
	   ...
	   */
          ...
          if(a){
                   $(".single-product .woocommerce-product-gallery .wp-post-image").attr("src", a);
	                ...
	                $(".single-product .woocommerce-product-gallery__image a").removeAttr("href");
	                $(".single-product .woocommerce-product-gallery .wp-post-image").attr("src", "https://vitalaunch.io/wp-content/uploads/2024/02/MicrosoftTeams-image-1.png");
                    $(".single-product .woocommerce-product-gallery .wp-post-image").attr("src", "https://vitalaunch.io/wp-content/uploads/2024/02/MicrosoftTeams-image-1.png");
	                $(".single-product .woocommerce-product-gallery .wp-post-image").attr("data-src", "https://vitalaunch.io/wp-content/uploads/2024/02/MicrosoftTeams-image-1.png");
	                $(".single-product .woocommerce-product-gallery .wp-post-image").attr("data-large_image", "https://vitalaunch.io/wp-content/uploads/2024/02/MicrosoftTeams-image-1.png");
	                $(".single-product .woocommerce-product-gallery .wp-post-image").attr("srcset", "https://vitalaunch.io/wp-content/uploads/2024/02/MicrosoftTeams-image-1.png");
	                $(".single-product .woocommerce-product-gallery .wp-post-image a").attr("href", "https://vitalaunch.io/wp-content/uploads/2024/02/MicrosoftTeams-image-1.png");
   ...
   // if($productID == "4746"){
	...
	<div class="row cus_button">
	 ...
	 <div class="con_content_1">
	    ...
	    <p>Pick your tool below to customize the label template </p>
	    <div class="canvasDiv">
            <a target="_blank" href="https://www.canva.com/design/DAF6sfO0Pbw/D_vLEd8lVrd5ZqorPGp_eA/view?utm_content=DAF6sfO0Pbw&utm_campaign=designshare&utm_medium=link&utm_source=publishsharelink&mode=preview" class="canvaBtn">Canva editor</a><br>
          </div>  
        <b>Step 2: Upload label PDF</b>
        <p>Upload a print quality PDF file. The product image above will refresh automatically</p>
...
{ 
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Merge static image with the uploaded image
            //mergeImages1($staticImagePath, $targetFile2, $mergedFile);
           // mergeImages($staticImagePath, $targetFile2, $mergedFile, 3000, 3000);
           // mergeImages($staticImagePath, $targetFile2, $mergedFile, 2.5);
...
function mergeImages($staticImagePath, $imagePath2, $outputPath, $zoomFactor = 1.0, $marginTop = 0) {
    ...
    $positionX = ($image1Width - $image2Width * $zoomFactor) / 2;
...
function custom_new_product_image( $_product_img, $cart_item, $cart_item_key ) {
   ...
   $product_id = $cart_item['product_id'];
    ...
    if($quantity >= 50){
        ...
        foreach($post_ids as $post_id){
            ?><img src='https://vitalaunch.io/wp-content/uploads/<?php echo $post_id->image_url; ?>' /><?php
        }
...
function options_on_single_product(){
    ?>
        <label for="label_price">
            <input type="hidden" name="label_price" class="label_price"  value="">  
...
function add_custom_text_to_order_details_page($order_id) {
    ...
    echo '<p>This is your custom text. You can replace it with any content you want.</p>';
}
